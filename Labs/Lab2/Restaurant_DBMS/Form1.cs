...
namespace Restaurant_DBMS
{
    public partial class Form1 : Form
    {
        ...
        {
            if (childTableView.SelectedRows.Count > 0)
            {
                foreach (DataGridViewRow row in childTableView.SelectedRows)
                {
                    DataRowView rowView = (DataRowView)childTableView.Rows[row.Index].DataBoundItem;
                    rowView.Row.Delete();
                }
            }
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            // Assuming child table has specific columns that need initialization
            DataTable childTable = tableData.Tables[getChildTableName()];
            DataRow newRow = childTable.NewRow();

            // Initialize newRow with default or empty data suitable for your schema
            newRow["Column1"] = "DefaultValue1";
            newRow["Column2"] = 0; // Default numeric value
                                    // Add more columns as necessary

            // Add the new row to the DataTable
            childTable.Rows.Add(newRow);

            // Optionally, select the newly added row in the DataGridView
            childTableView.ClearSelection();
            childTableView.Rows[childTableView.Rows.Count - 1].Selected = true;
